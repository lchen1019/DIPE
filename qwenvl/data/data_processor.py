...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, processor, data_args):
        ...
        rng.shuffle(list_data_dict)  # Randomly shuffle the data for training
    ...
    def _get_item(self, sources) -> Dict[str, torch.Tensor]:
        ...
        position_ids, static_position_ids, _ = self.get_rope_index(
            self.merge_size,
            data_dict["input_ids"],
        ...
        data_dict["static_position_ids"] = static_position_ids
    ...
    def _get_packed_item(self, sources) -> Dict[str, torch.Tensor]:
        ...
        if isinstance(sources, list):
            ...
            static_position_ids = torch.cat([d["static_position_ids"] for d in data_list], dim=2)
            attention_mask = [
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        input_ids, labels, position_ids, static_position_ids  = tuple(
            [instance[key] for instance in instances]
            for key in ("input_ids", "labels", "position_ids", "static_position_ids")
        )
        ...
        static_position_ids = static_position_ids[:, :, : self.tokenizer.model_max_length]
        batch = dict(
            input_ids=input_ids,
            labels=labels,
        ...
        batch["static_position_ids"] = static_position_ids
        return batch
...
class FlattenedDataCollatorForSupervisedDataset(DataCollatorForSupervisedDataset):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        input_ids, labels, position_ids, static_position_ids, attention_mask = tuple(
            [instance[key] for instance in instances]
            for key in ("input_ids", "labels", "position_ids", "static_position_ids", "attention_mask")
        )
        attention_mask = list(
            itertools.chain(
                *(
        ...
        static_position_ids = torch.cat(static_position_ids, dim=2)
